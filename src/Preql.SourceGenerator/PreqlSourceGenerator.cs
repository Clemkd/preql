...
namespace Preql.SourceGenerator
{
    ...
    ///   <item>The complete SQL format string is pre-computed for every SQL dialect and
    ///         embedded as a string literal in the generated source.</item>
    /// </list>
    ///
    /// <para>What happens at <b>runtime</b> (per call):</para>
    /// <list type="bullet">
    ///   <item>For queries with no parameters: a single array-index lookup returns the
    ///         pre-built <see cref="System.FormattableString"/> — zero allocations.</item>
    ///   <item>For queries with parameters: the pre-computed format string is looked up by
    ///         dialect index; only parameter value extraction and
    ///         <c>FormattableStringFactory.Create</c> run at call time.</item>
    /// </list>
    /// </summary>
    [Generator]
    public class PreqlSourceGenerator : IIncrementalGenerator
    {
        ...
        private static string GenerateInterceptor(QueryInvocationInfo info)
        {
            ...
            // Pre-compute the full SQL format string for every dialect at code-generation time.
            ...
            // ── Pre-computed static arrays (one entry per SqlDialect) ─────────────
            if (!hasParams)
            {
                // Entire FormattableString objects created once at class-load time.
            ...
            {
                // Format strings with {n} placeholders; no runtime string building needed.
            ...
            if (!hasParams)
            {
                // ── No runtime parameters: pure array-index lookup ────────────────
            ...
            {
                // ── Runtime parameters: lookup format, extract params ─────────────
                sb.AppendLine("        var __di = (int)context.Dialect;");
